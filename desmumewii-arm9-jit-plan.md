...
| **A4-P1/P2** 🟡 (`4c186e3`) | **Block chaining + the scheduler quota.** `JIT_ENABLE_CHAINING` flips 0→1: a static-target exit (B/BL, non-S data-proc→pc, the scanner's own fall-through) BLs to the shared linker stub instead of always returning to C; on a hit the stub self-patches *that call site* to a direct branch into the target block's arena code — sound because a static exit's call site always wants the one compile-time-constant target baked into it. `emitDynamicExit` (BX/LDR pc/POP{pc}) deliberately does not chain (the same call site can resolve to a different PC every visit). This also delivers P2 (`armcpu_exec_block(quota)`) for free: `JIT_YIELD_NUMBER` was already a cross-block cycle quota threaded through r3 by `ensureArena()`'s guard + `emitAddCycles()` at every exit, just gated off — chaining lets a call now run ~64 guest cycles inside compiled code before yielding, no new scheduler code needed. **Harness fix required to validate it:** `jitRunChecked()`'s bounded reference loop (≤ `block->insnCount()`) can no longer see a chained dispatch's true instruction count, so it now runs a *trial* JIT pass first (journalled + rolled back, like the reference pass) to learn the real chain length before anything touches memory for real, then an extended free-running interpreter comparison against the trial's result. (First attempt compared against the *real* run's already-mutated memory instead of a trial's, producing 3 false CHAIN-DIFFs from a read-modify-write SWP spinlock helper reading back what the real run had just written — same class of bug A1's journal exists to prevent, just introduced fresh in the wrong pass order.) | ✅ PH soak 210s: 0 DIFF / 0 ARENA OVERRUN, both selftests PASS, 53.7M blk / 189M insns, extended check re-verifies ~26.7M chained dispatches (countDiv) the bounded check can't see, 0 mismatches. vsd soak 180s: 0 DIFF, 100K blk / 588K insns. JIT-off byte-identical. Benchmark: vsd JIT realtime% 8.6%→13.7% (+60%), still ~1.8× slower than interpreter. **PH `jit9on` still produces 0 bench frames in 150s — chaining alone does not fix the hang.** PH's hot ARM loop includes a cross-core `SWP R0,R0,[R1]` spinlock at `0200c630` ("wait for ARM7 to release a semaphore") — plausibly sensitive to `armInnerLoop` interleave granularity (chaining coarsens how often ARM9 yields back to let ARM7 run), not just per-block trampoline cost. Not yet root-caused. |
| **A4-P3/P4** ⛔ BLOCKED | Inline main-RAM load/store fast path (v2, never built); skip the full 18-register save/restore for small blocks. Blocked behind root-causing the PH spinlock-interleave hang above — no point tuning further until PH itself is not hanging. | PH `jit9on` reaches gameplay at ≥ interpreter speed; multi-ROM differential soak; hardware validation of the arena; only then `jitArm9Enabled` → default-on |
