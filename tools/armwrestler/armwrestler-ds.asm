...
@ ===========================================================================
@ §17 auto-run driver (desmumewii): the stock interactive menu/keys loop is
@ replaced with a straight-line walk over every ARM test group (RunTest is
@ already a clean tail-dispatcher: Test0..Test5 each stmfd/ldmfd their own
@ lr, so calling it repeatedly via `bl RunTest` from here is exactly what the
@ stock forever loop did on each START press) and every THUMB test group
@ (calling _runtest directly, the same tail-call shape _tmbmain itself uses,
@ so _tmbmain's own non-returning interactive loop is never entered). Every
@ DrawResult / _drawresult call is unchanged -- see the AW_RecordResult hook
@ inserted at its entry -- so the pass/fail arithmetic is byte-identical to
@ stock, just harvested into slot-2 expansion RAM instead of read off-screen.
@ `forever` stays defined (thumbwrestler's dead _menu handler still names it)
@ but is now just the terminal spin.
@ ===========================================================================
	ldr 	r0,=0x09000000
	...
	@ ---- ARM: TESTNUM 0..5 (Test0..Test5). Fully unrolled rather than
	@ looped in a register -- RunTest's targets are free to clobber any
	@ scratch reg (r8/r9 included; the stock forever loop never needed one
	@ to survive a `bl RunTest`), so a loop counter kept in a register
	@ would be unsound. mov r8,#16 matches the stock per-iteration setup. ----
	mov 	r8,#16
	mov 	r1,#0
	bl 	RunTest
	mov 	r8,#16
	...
	@ ---- THUMB: _runtest 0..1 (_test2 is header-only, no real test).
	@ _runtest's own calling convention (push{lr} then tail-bx into _testN,
	@ which pop{pc}s straight back to us) is exactly what a `bl`-from-ARM
	@ needs -- see thumbwrestler-ds.asm. r5=1 / r7=16 match _tmbmain's own
	@ preamble (TEST_Rd's bitmask math reads r5 as its "1" literal); same
	@ no-register-loop-counter reasoning as the ARM side above. ----
	mov 	r5,#1
	mov 	r7,#16
	mov 	r1,#0
	ldr 	r0,=(_runtest+1)
...
@ ---------------------------------------------------------------------------
@ §17 auto-run driver: tally one ARM test result into slot-2 expansion RAM
@ (0x09000000+, the ExpMemory addon -- see the emulator-side hook). Layout:
...
@   +0x18.. {namePtr:u32, rawBitmask:u32} x32, name -> the szXXX asciz label
@   itself (still resolvable, it's a live NDS-memory address the host can
@   read back), bitmask raw per architecture (ARM vs THUMB encode it
@   differently -- not unified, the host just reports both as hex + name).
@ Fail predicate mirrors DrawResult's own `tsts r4,#0xFF; beq dr_ok` exactly,
@ so this is byte-identical bookkeeping to what the screen already shows, not
@ a second, potentially-diverging notion of pass/fail. r0-r5 fully preserved.
@ ---------------------------------------------------------------------------
AW_RecordResult:
