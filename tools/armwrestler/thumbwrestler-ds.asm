...
.text
.global _tmbmain
.global _runtest		@ §17 auto-run driver (armwrestler-ds.asm) calls this directly
.thumb
...
@ ---------------------------------------------------------------------------
@ §17: tally one THUMB test result into slot-2 expansion RAM (see the ARM-side
@ AW_RecordResult in armwrestler-ds.asm for the header layout / rationale --
@ this is its THUMB twin, sharing the same fail-log region). r0 = the test's
@ name-string pointer, r6 = bitmask (nonzero == fail); both fully preserved.
@ ---------------------------------------------------------------------------
_AW_Record:
	push 	{r0-r6,lr}
...
@ r6: bitmask
	...
	bl 	_AW_Record		@ §17: tally into slot-2 (r0/r6 preserved)
