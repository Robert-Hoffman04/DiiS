...
mapcol:
    ...
    @ (white) -- a 16x16 sprite is a 2x2 tile grid, and with 1D OBJ mapping
    @ (DISPCNT bit6, set below) tile indices 2,3 (top row) and 4,5 (bottom
    @ row) are contiguous, so one 128-byte fill covers all four (OBJ char
    @ base is fixed at VRAM+0x10000). 2D mapping's non-contiguous per-row
    @ tile layout is why an earlier version of this test only filled one
    @ 32-byte tile and left 3/4 of the sprite reading unwritten (transparent)
    @ VRAM -- a test-asset bug, not a renderer bug.
    ldr r0, =0x06010040
    ldr r1, =0x11111111
    mov r2, #32
objtilefill:
    ...
    @ OAM entry 0: the affine sprite -- 16x16 (shape0/size1), matrix group 0,
    @ tile 2, palette 0, priority 0, at screen (100,80)
    ldr r0, =0x07000000
    mov r1, #(1 << 8)
    add r1, r1, #80
    strh r1, [r0]
