#!/usr/bin/env bash
# assoc-ab.sh - A/B the 2-way set-associative block table (jit_cache.h/.cpp,
# BLOCK_TABLE_WAYS) against the committed 1-way direct-mapped table, using
# -DJIT_HASH_HISTO over the unified harness's PKT_PROFILE sink. baseline =
# the associativity change stashed out, after = working tree as-is.
#
# Requires: Dolphin's Wii network passthrough already enabled (plan §0).
#
# Usage: tools/benchmark/assoc-ab.sh [seconds]   (default 150)
set -uo pipefail
...
RESDIR="$HERE/results/_assoc_$(date -u +%Y%m%dT%H%M%SZ)"
mkdir -p "$RESDIR"
TESTDEFS="-DDESMUME_FORCE_ROM -DDESMUME_BENCH -DDESMUME_BENCH_FRAMES=200000 -DDESMUME_FORCE_CORE=1 -DDESMUME_AUTOLOADSTATE"
BASEDEFS="-DDESMUME_JIT_ARM7 -DDESMUME_JIT_ARM9_ON -DDESMUME_HARNESS -DHARNESS_TRANSPORT_NET -DJIT_HASH_HISTO"

ASSOC_STASH_FILES="source/src/jit/jit_cache.h source/src/jit/jit_cache.cpp source/src/jit/jit_debug.h source/src/jit/jit_trace.cpp"
...
build_and_run() {
	local tag="$1"
	echo ">> [$tag] build (JITDEFS: $BASEDEFS)"
	( cd "$ROOT" && make clean >/dev/null 2>&1 && make -j"$(nproc)" JITDEFS="$BASEDEFS" TESTDEFS="$TESTDEFS" ) >"$RESDIR/_build_$tag.log" 2>&1 \
...
echo "== 1-way (committed, associativity change stashed out) =="
git -C "$ROOT" stash push -q -m assoc-ab-baseline -- $ASSOC_STASH_FILES
build_and_run 1way
git -C "$ROOT" stash pop -q

echo "== 2-way (working tree as-is) =="
build_and_run 2way

echo
echo "==== ARM9 hashhisto summary (last lines) ===="
for tag in 1way 2way; do
